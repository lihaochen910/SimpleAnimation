...
public partial class SimpleAnimation: MonoBehaviour
{
    ...
    [SerializeField]
    private SimpleAnimationGroup m_AnimationGroup;
    //private EditorState[] m_States;
    public SimpleAnimationGroup AnimationGroup
    {
        get { return m_AnimationGroup; }
        set
        {
            if (value == null || value == m_AnimationGroup)
                return;

            //DateTime beforDT = System.DateTime.Now;

            m_Playable.StopAll();
            m_Playable.RemoveAllClips();
            m_Playable = null;

            m_AnimationGroup = value;
            ...
            //Debug.Log("Switch AnimationGroup Cost:" + ts.TotalMilliseconds + "ms");
        }
    }
    ...
    private void Initialize()
    {
        ...
        if (m_AnimationGroup != null)
        {
            foreach (var state in m_AnimationGroup.AnimationStates)
            {
                if (state.clip)
                {
    ...
    private void RebuildStates()
    {
        ...
        m_AnimationGroup.AnimationStates = list.ToArray();
    }
    ...
    private void OnValidate()
    {
        ...
        if (Application.isPlaying || AnimationGroup == null)
            return;
        ...
        if (AnimationGroup.AnimationStates == null || AnimationGroup.AnimationStates.Length == 0)
        {
            AnimationGroup.AnimationStates = new EditorState[1];   
        }

        //Create default state if it's null
        if (AnimationGroup.AnimationStates[0] == null)
        {
            AnimationGroup.AnimationStates[0] = CreateDefaultEditorState();
        }

        //If first state is not the default state, create a new default state at index 0 and push back the rest
        if (AnimationGroup.AnimationStates[0].defaultState == false || AnimationGroup.AnimationStates[0].name != "Default")
        {
            var oldArray = AnimationGroup.AnimationStates;
            AnimationGroup.AnimationStates = new EditorState[oldArray.Length + 1];
            AnimationGroup.AnimationStates[0] = CreateDefaultEditorState();
            oldArray.CopyTo(AnimationGroup.AnimationStates, 1);
        }

        //If default clip changed, update the default state
        if (AnimationGroup.AnimationStates[0].clip != m_Clip)
            AnimationGroup.AnimationStates[0].clip = m_Clip;


        //Make sure only one state is default
        for (int i = 1; i < AnimationGroup.AnimationStates.Length; i++)
        {
            if (AnimationGroup.AnimationStates[i] == null)
            {
                AnimationGroup.AnimationStates[i] = new EditorState();
            }
            AnimationGroup.AnimationStates[i].defaultState = false;
        }

        //Ensure state names are unique
        int stateCount = AnimationGroup.AnimationStates.Length;
        string[] names = new string[stateCount];

        for (int i = 0; i < stateCount; i++)
        {
            EditorState state = AnimationGroup.AnimationStates[i];
